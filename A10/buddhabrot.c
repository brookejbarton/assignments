...
void color_comp(struct mandel_pack *pack){
  ...
  for (int i = pack->start_row; i < pack->end_row; i++){ 
    for (int j = pack->start_col; j < pack->end_col; j++){
        float value = 0;
    ...
    for (int j = pack->start_col; j < pack->end_col; j++){
      if (pack->escaped[i*pack->size+j] == false){
        ...
        while (x*x + y*y < 2*2){
          ...
          int yrow = round(pack->size * (y - pack->ymin)/(pack->ymax - pack->ymin));
          int xcol = round(pack->size * (x - pack->xmin)/(pack->xmax - pack->xmin));

          
          
           
          if (yrow >= 0 && yrow < pack->size){ // in range
            if (xcol >= 0 && xcol < pack->size) { // in range  
              int idx = yrow*pack->size+xcol;
              pthread_mutex_lock(&mutex);
              pack->count[idx]++;
              
              if (pack->count[idx] > max){
                max = pack->count[idx];
              }
...
int main(int argc, char* argv[]) {
  ...
  bool *escaped = calloc((size)*(size), sizeof(bool));
  int *count = calloc((size)*(size), sizeof(int));
  ...
  for (int i = 0; i < numProcesses; i ++){
    ...
    q_pack[i].ymin = ymin;
    q_pack[i].ymax = ymax;
    q_pack[i].maxIterations = maxIterations;
    q_pack[i].size = size;
    q_pack[i].escaped = escaped;
    q_pack[i].count = count;
  }

  //quadrant 1
    q_pack[0].start_col = 0;
    q_pack[0].end_col = (size/2);
    q_pack[0].start_row = 0;
    q_pack[0].end_row = (size/2);
    q_pack[0].to_pass = to_pass;

  //quadrant 2
    q_pack[1].start_col = (size/2);
    q_pack[1].end_col = size;
    q_pack[1].start_row = 0;
    q_pack[1].end_row = (size/2);
    q_pack[1].to_pass = to_pass;

  //quandrant 3
    q_pack[2].start_col = 0;
    q_pack[2].end_col = (size/2);
    q_pack[2].start_row = (size/2);
    q_pack[2].end_row = size;
    q_pack[2].to_pass = to_pass;

  //quadrant 4 
    q_pack[3].start_col = (size/2);
    q_pack[3].end_col = size;
    q_pack[3].start_row = (size/2);
    q_pack[3].end_row = size;
